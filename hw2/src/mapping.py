...
class mapping:
    ...
    def start(self):
        ...
        while not rospy.is_shutdown():
            img = self.update_map()
            cv2.imshow("Mapping - HW2", img)
            cv2.waitKey(1)
            cv2.imwrite('/home/rtk-noetic/rob_hw/src/mobile_robotics_hw/hw2/map/map.jpg', img*200)
            cv2.waitKey(2)
    ...
    def pose(self, pose_data):
        ...
        # rospy.loginfo(f'Published {self.pose_data}')
    ...
    def update_map(self):
        ...
        for (dist, ang) in zip(distance, angle):
            dists_x = np.append(dists_x, odomx + dist * np.cos(ang + odomtheta - np.pi/2))
            dists_y = np.append(dists_y, odomy + dist * np.sin(ang + odomtheta - np.pi/2))
